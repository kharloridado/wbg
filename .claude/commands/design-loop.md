...
2. Delegate to @maker to implement it.
3. Delegate to @checker to validate.
   - PASS -> status "built"; commit on the loop branch (outsystems-git-helpers message); ensure the component's GitHub issue exists (dedup: search for `[node:<id>]`; create only if absent) as type Component/handover; add it to the GitHub Project (`gh project item-add <num> --owner <owner> --url <issue-url>`); attach to the tier's handover epic as a sub-issue (`gh issue edit <child> --parent <epic>`); update the Style Guide page.
   - FAIL -> feed the CRITIQUE to @maker, increment the item's round counter; if over caps.max_rounds_per_item, status "needs-human" with the critique, move on.
4. For every finding: dedup by `[node:<id>]`, then file a GitHub Bug (type Bug + labels finding,bug,<type>,sev:*), add it to the Project. NEVER resolve a brand/a11y conflict.
5. Persist state.json after every item; increment iteration.
